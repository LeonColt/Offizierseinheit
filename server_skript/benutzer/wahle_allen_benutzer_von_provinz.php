...
class wahle_allen_benutzer_von_provinz extends wahle_allen_benutzer
{
    private $provinz;
...
JOIN user_tingkat_provinsi ON sg_user.id = user_tingkat_provinsi.id AND user_tingkat_provinsi.id_provinsi = ?
WHERE sg_user.id_jenis = 3  AND sg_user.active = 1
ORDER BY sg_user.id LIMIT '.$this->seite.', '.anzaigen_enstellung_benutzer_seite_max_zeile;
        $this->parameter = array($this->provinz);
    }
    public function satz_provinz($provinz)
    {
