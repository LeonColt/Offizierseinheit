 ...
 * Time: 16:15
 */
class diagramme_wahle_allen_modal_pinjaman_von_datum_provinz_kabupaten_per_model extends  Base_diagramme_1_und_4
{
...
IFNULL((SELECT SUM(modal_pinjaman) FROM '.daten_bank_name.'.rekap WHERE tanggal = ? AND id_propinsi = ? AND id_kabupaten_kota = ? AND id_model = id),0) AS total_satgas,
IFNULL((SELECT SUM(tblindikatorusahadetail.ModalLuar) FROM tblkoperasi JOIN tblindikatorusahadetail ON tblkoperasi.IdKoperasi = tblindikatorusahadetail.IdKoperasi AND tblindikatorusahadetail.Tahun = ? WHERE IdPropinsi = ? and IdKabupaten = ? AND IdModelKoperasi = id),0) AS total_data_individual
