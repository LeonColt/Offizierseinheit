...
class diagramme_wahle_allen_koperasi_aktif_von_datum_propinsi_kabupaten_per_model extends Base_diagramme_1_und_4
{
...
IFNULL((SELECT SUM(jumlah_koperasi_aktif) FROM '.daten_bank_name.'.rekap WHERE tanggal = ? AND id_propinsi = ? AND id_kabupaten_kota = ? AND id_model = id), 0) AS total_satgas,
IFNULL((SELECT COUNT(IdKoperasi) FROM tblkoperasi WHERE CreatedDate < ? AND IdPropinsi = ? AND IdKabupaten = ? AND aktif = 1 AND IdModelKoperasi = id), 0) AS total_data_individual
