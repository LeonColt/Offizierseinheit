 ...
 * Time: 10:11
 */
class diagramme_wahle_allen_pinjaman_diberikan_von_datum_provinz_kabupaten_per_model extends Base_diagramme_1_und_4
{
...
IFNULL((SELECT SUM(pinjaman_diberikan) FROM '.daten_bank_name.'.rekap WHERE tanggal = ? AND id_propinsi = ? AND id_kabupaten_kota = ? AND id_model = id),0) AS total_satgas,
IFNULL((SELECT SUM(tblindikatorpinjaman.PinjamanLancar) + SUM(tblindikatorpinjaman.PinjamanKurangLancar) + SUM(tblindikatorpinjaman.PinjamanDiragukan) + SUM(tblindikatorpinjaman.PinjamanMacet) FROM tblkoperasi JOIN tblindikatorpinjaman ON tblkoperasi.IdKoperasi = tblindikatorpinjaman.IdKoperasi AND tblindikatorpinjaman.Tahun = ? WHERE IdPropinsi = ? and IdKabupaten = ? AND IdModelKoperasi = id),0) AS total_data_individual
