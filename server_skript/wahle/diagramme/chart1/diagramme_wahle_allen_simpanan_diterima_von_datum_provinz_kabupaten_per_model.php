 ...
 * Time: 10:11
 */
class diagramme_wahle_allen_simpanan_diterima_von_datum_provinz_kabupaten_per_model extends Base_diagramme_1_und_4
{
...
IFNULL((SELECT SUM(simpanan_diterima) FROM '.daten_bank_name.'.rekap WHERE tanggal = ? AND id_propinsi = ? AND id_kabupaten_kota = ? AND id_model = id),0) AS total_satgas,
IFNULL((SELECT SUM(tblindikatorsimpanan.SimpananAnggota) + SUM(tblindikatorsimpanan.SimpananBukanAnggota) FROM tblkoperasi JOIN tblindikatorsimpanan ON tblkoperasi.IdKoperasi = tblindikatorsimpanan.IdKoperasi AND tblindikatorsimpanan.Tahun = ? WHERE IdPropinsi = ? and IdKabupaten = ? AND IdModelKoperasi = id),0) AS total_data_individual
