...
class excel_schichten_provinz extends Base_ausgabe_provinz implements Interface_excel_schichten
{
    ...
    public function __construct() {
        ...
        $this->excel->getProperties()->setTitle('REKAPITULASI '.filter_input(INPUT_GET, ausgabe_parameter_model_zeichenfolge).' PRIMER DAN SEKUNDER NASIONAL BERDASARKAN PROVINSI PER TAHUN '.filter_input(INPUT_GET, ausgabe_jahr).' Bulan '.$this->ausgabe_erhalten_monat_fur_title(filter_input(INPUT_GET, ausgabe_monat)));
        $this->zeiger_zeile = 1;
    ...
    public final function ausfuhren()
    {
        ...
        header('Content-Disposition: attachment;filename="REKAPITULASI '.filter_input(INPUT_GET, ausgabe_parameter_model_zeichenfolge).' PRIMER DAN SEKUNDER NASIONAL BERDASARKAN PROVINSI PER TAHUN '.filter_input(INPUT_GET, ausgabe_jahr).' BULAN '.$this->ausgabe_erhalten_monat_fur_title(filter_input(INPUT_GET, ausgabe_monat)).'.xlsx"');
    ...
    public final function satz_header()
    {
        ...
        $this->excel->getActiveSheet()->SetCellValue('H'.$this->zeiger_zeile++, 'REKAPITULASI '.filter_input(INPUT_GET, ausgabe_parameter_model_zeichenfolge));
        $this->excel->getActiveSheet()->setCellValue('I'.$this->zeiger_zeile++, 'PRIMER DAN SEKUNDER NASIONAL');
        $this->excel->getActiveSheet()->setCellValue('I'.$this->zeiger_zeile++, 'BERDASARKAN PROVINSI');
        $this->excel->getActiveSheet()->SetCellValue('I'.$this->zeiger_zeile++, 'POSISI PER TAHUN '.filter_input(INPUT_GET, ausgabe_jahr).' BULAN '.$this->ausgabe_erhalten_monat_fur_title(filter_input(INPUT_GET, ausgabe_monat)));
        $this->zeiger_zeile++;
        $this->zeiger_zeile++;
        $this->beginnen_zeile = $this->zeiger_zeile;
    ...
    protected final function zeichnen_daten()
    {
        ...
        {
            $this->excel->getActiveSheet()->setCellValue($this->header[0].$this->zeiger_zeile, ($i+1));
            $this->excel->getActiveSheet()->setCellValue($this->header[1].$this->zeiger_zeile, $this->provinz[$i][1]);
            for($j = 0; $j<$this->insgesamt_spalte; $j++)
            {
                $this->excel->getActiveSheet()->setCellValue($this->header[$j+2].$this->zeiger_zeile, $this->erhalten_wert($daten_insgesamt_von_provinz[$this->provinz[$i][0]][$j]));
            }
            $this->zeiger_zeile++;
            for($j = 0; $j<count($this->bentuk); $j++)
            {
                ...
                {
                    $this->excel->getActiveSheet()->setCellValue($this->header[$k+2].$this->zeiger_zeile, $this->erhalten_wert($daten_zeichnen[$this->provinz[$i][0]][$this->bentuk[$j][0]][$k]));
                }
                $this->zeiger_zeile++;
            }
        }
